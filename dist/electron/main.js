...
// Crypto bridges
ipcMain.handle('crypto:decrypt', (_evt, cipherText, password, salt) => {
    return springDecrypt(cipherText, password, salt);
});
ipcMain.handle('crypto:parse-and-decrypt', (_evt, data, password, salt) => {
...
ipcMain.handle('crypto:re-encrypt', (_evt, current, rememberedKeys, password, salt) => {
...
ipcMain.handle('crypto:parse-keys-only', (_evt, data) => {
